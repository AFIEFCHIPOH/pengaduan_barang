...
function submit(x) {
    if (x == 'add') {
        ...
        $('#barangkeluarModal .modal-title').html('Edit barang keluar');
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            ...
            <a href="<?=base_url();?>process/cetak_laporan_transaksi_barang_today.php?jenis_transaksi=masuk" target="_blank"
                ...
                <span class="text">Cetak barang keluar hari ini</span>
            </a>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php 
                        $n=1;
                        $query = mysqli_query($con,"SELECT tb_transaksi.*, tb_barang.nama_barang, tb_barang.keterangan_barang, tb_merek.nama_merek, tb_kategori.nama_kategori, tb_kondisi.nama_kondisi, tb_ruangan.nama_ruangan  FROM tb_transaksi LEFT JOIN tb_barang ON tb_transaksi.id_barang = tb_barang.id_barang LEFT JOIN tb_merek ON tb_merek.id_merek = tb_barang.id_merek LEFT JOIN tb_kategori ON tb_kategori.id_kategori = tb_barang.id_kategori LEFT JOIN tb_ruangan ON tb_ruangan.id_ruangan = tb_barang.id_ruangan LEFT JOIN tb_kondisi ON tb_kondisi.id_kondisi = tb_barang.id_kondisi WHERE jenis_transaksi = 'keluar' ORDER BY tanggal ASC")or die(mysqli_error($con));
...
<div class="modal fade" id="barangkeluarModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form action="<?=base_url();?>process/process_barang_keluar.php" method="post">
